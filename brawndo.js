...
module.exports.order.pickup_signature = function(params, callback) {
  if (!configured) {
    throw new Error('Call configure before calling the api');
  }
  ...
  if (params && params.company_id) {
    req = request
      .get(API_ORDER_URL + '/pickup_signature/' + params.order_id)
      .query({
        company_id : params.company_id
      });
  } else {
  ...
  req
    .set('Accept', 'application/json')
    .use(signing_mw(API_ORDER_PATH + '/pickup_signature/' + params.order_id, function(error, response){
      if (error) {
        callback(error);
      } else if (response.status === 200 && response.body) {
        callback(void(0), response.body);
